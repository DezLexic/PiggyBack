...
def get_project(project_id: UUID, conn: DbConn) -> ProjectRead:
...
def create_file(project_id: UUID, body: FileCreate, conn: DbConn) -> FileRead:
...
def list_files(project_id: UUID, conn: DbConn) -> list[FileRead]:
    if projects_repo.get_project(conn, project_id) is None:
        raise HTTPException(status_code=404, detail="Project not found")
